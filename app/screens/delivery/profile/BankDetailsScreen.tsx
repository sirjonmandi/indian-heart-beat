...
import { fetchBankDetails, UpdateBankDetails } from '@/store/slices/deliverySlice';
interface RootState {
  delivery: {
    user?: {
      phone: string;
...
const BankDetailsScreen: React.FC = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    if(profile)
    {
      updateField('partner_id',profile?.user.id);
    }
  }, [bankDetails,profile]);

  const fetchPartnerBankDetails = async () => {
    setLoading(true);
    try {
  ...
  const handleCancel = () => {
    if (bankDetails) {
      setFormData({
        partner_id:profile?.user.id || null,
        account_holder_name: bankDetails.account_holder_name || '',
        account_number: bankDetails.account_number || '',
        ifsc_code: bankDetails.ifsc_code || '',
        bank_name: bankDetails.bank_name || '',
        ...
        <View style={styles.profileHeader}>
          <View style={styles.profileInfo}>
            ...
            <Text style={styles.partnerCode}>
              Partner ID: {profile?.delivery_partner.partner_code ?? 'N/A'}
            </Text>
          </View>
        </View>
